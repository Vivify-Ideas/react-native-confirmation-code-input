...
export default class ConfirmationCodeInput extends Component {
  ...
  getFocusedStyles() {
    const {codeInputFocusedStyle, codeInputStyle} = this.props;
    if(codeInputFocusedStyle) {
      return codeInputFocusedStyle;
    }

    return codeInputStyle;
  }

  getStyle(fieldIndex) {
    const { currentIndex } = this.state;

    return  currentIndex === fieldIndex ? this.getFocusedStyles() : this.props.codeInputStyle
  }
  ...
  render() {
    const {
      ...
      className,
      size,
      activeColor,codeInputFocusedStyle
    } = this.props;
